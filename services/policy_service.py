...
class PolicyService:
    ...
    def get_per_diem_limit(self, country: str, category: str) -> Decimal | None:
        per_diem = self.policy["per_diem"]
        country_rules = per_diem.get(country, per_diem["DEFAULT"])
    ...
    def is_restricted_category(self, category: str) -> bool:
        return category in self.policy["categories"].get("restricted", [])

    def is_blocked_category(self, category: str) -> bool:
        return category in self.policy["categories"].get("blocked", [])

    def receipt_required_above(self) -> Decimal:
        return Decimal(self.policy["receipts"]["required_above"])
    ...
    def get_confidence_threshold(self) -> float:
        return float(self.policy["confidence"]["manual_review_below"])

    def get_rule(self, rule_name: str) -> dict:
        return self.policy["policy_rules"][rule_name]

    def get_alcohol_blocked_countries(self) -> list[str]:
        return self.policy["policy_rules"]["alcohol_blocked_country"][
            "blocked_countries"
        ]

    def meals_require_weekend_approval(self) -> bool:
        return bool(self.policy["weekend"]["meals_require_approval"])
    ...
    def is_vat_eligible_country(self, country: str) -> bool:
    ...
    def is_fast_track_enabled(self) -> bool:
        return bool(self.policy["fast_track"]["enabled"])

    def get_fast_track_limit(self) -> Decimal:
        return Decimal(self.policy["fast_track"]["below"])
